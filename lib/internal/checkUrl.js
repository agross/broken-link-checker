...
function checkUrl(link, baseUrl, cache, options, retry)
{
	...
	var request = promiseRetry({ retries: 5 }, function (retry, number) {
		if (number > 1)
		{
			console.warn('Retrying #' + number + ': ' + link.url.resolved);
		}

		return bhttp.request(link.url.resolved,  // TODO :: https://github.com/joepie91/node-bhttp/issues/3
		{
...
function copyResponseData(response, link, options)
{
	...
	else
	{
		...
		{
			link.brokenReason = response.name + ': ' + response.message;
		}
		else
		{
